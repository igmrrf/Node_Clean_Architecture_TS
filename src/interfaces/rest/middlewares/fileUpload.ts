...
type MyFunction = (arg1: null | Error, arg2?: boolean | string) => number;
...
const fileFilter =
  (fileType = "image") =>
  (req: Request, file: any, cb: MyFunction) => {
    switch (fileType) {
      case "image":
...
const uploader =
  ({ fileType, fieldName, fields }: { fileType: string; fieldName: string; fields: readonly Field[] }) =>
